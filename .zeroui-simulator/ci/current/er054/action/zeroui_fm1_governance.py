...
def license_evidence_context(evidence: Any) -> Dict[str, Any]:
    ...
    denied_license_count = data.get("denied_license_count", 0)
    try:
        denied_license_count = int(denied_license_count)
    except (TypeError, ValueError) as exc:
        raise SystemExit(f"denied_license_count must be an integer: {exc}") from exc

    allowed_license_count = data.get("allowed_license_count", 0)
    try:
        allowed_license_count = int(allowed_license_count)
    except (TypeError, ValueError):
        allowed_license_count = 0

    unknown_license_count = data.get("unknown_license_count", 0)
    try:
        unknown_license_count = int(unknown_license_count)
    except (TypeError, ValueError):
        unknown_license_count = 0

    return {
        "scan_type": str(data.get("scan_type") or "license"),
        "license_scanner": str(data.get("license_scanner") or "pip-licenses"),
        "license_report_format": str(data.get("license_report_format") or "pip-licenses-json"),
        "package_name": str(data["package_name"]),
        "package_version": str(data["package_version"]),
        "license_id": str(data["license_id"]),
        "license_text": str(data.get("license_text") or data["license_id"]),
        "policy_verdict": str(data["policy_verdict"]),
        "denied_license_count": denied_license_count,
        "allowed_license_count": allowed_license_count,
        "unknown_license_count": unknown_license_count,
        "scan_passed": bool(data["scan_passed"]),
        "license_policy_passed": bool(data.get("license_policy_passed", data["scan_passed"])),
...
def coverage_context(evidence: Any) -> Dict[str, Any]:
    ...
    return {
        ...
        "covered_file": data.get("covered_file", ""),
...
SUPPORTED_EVENT_BY_FORMAT = {
    ...
    "license-evidence": "ci.license_policy.failed",
}
...
def context_from_evidence(evidence_file: Path, evidence_format: str) -> Tuple[str, Dict[str, Any]]:
    ...
    if fmt == "license-evidence":
        return "ci.license_policy.failed", license_evidence_context(read_json(evidence_file))
...
def main() -> None:
    ...
    print(json.dumps({
        ...
        "denied_license_count": context.get("denied_license_count"),
