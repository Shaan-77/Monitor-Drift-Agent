...
def policy_gate_context(evidence: Any) -> Dict[str, Any]:
    ...
    if not isinstance(data["policy_gate_family"], str):
        raise SystemExit("policy_gate_family must be a string")
    if not isinstance(data["check_family"], str):
        raise SystemExit("check_family must be a string")
    if not isinstance(data["contract_check_failed"], bool):
        raise SystemExit("contract_check_failed must be a boolean")
    if not isinstance(data["policy_gate_id"], str):
        raise SystemExit("policy_gate_id must be a string")
    if not isinstance(data["policy_scope"], str):
        raise SystemExit("policy_scope must be a string")
    if not isinstance(data["policy_engine"], str):
        raise SystemExit("policy_engine must be a string")
    if not isinstance(data["required_gate"], bool):
        raise SystemExit("required_gate must be a boolean")
