...
SCENARIO_PATH = Path(".zeroui-simulator/ci/current/er067/scenario.json")
EXPECTED_ER = "ER-067"
...
def parse_junit_xml(text: str) -> Dict[str, Any]:
    try:
        ...
        raise SystemExit("ER067_JUNIT_EVIDENCE_INVALID") from exc

    testsuites: List[ET.Element] = []
    if root.tag == "testsuites":
        testsuites = [row for row in root.findall("testsuite")]
    elif root.tag == "testsuite":
        testsuites = [root]
    else:
        raise SystemExit("ER067_JUNIT_EVIDENCE_INVALID")
    ...
    failed_cases = [row for row in testcases if row["status"] == "failed"]
    if not failed_cases:
        raise SystemExit("ER067_EXPECTED_TEST_FAILURE_NOT_OBSERVED")
    ...
    return {
        "test_total": len(testcases) if testcases else totals["tests"],
        ...
        "failed_test_name": str(primary.get("name") or CONTROLLED_TEST_NAME),
        "failed_test_file": str(primary.get("file") or primary.get("classname") or FIXTURE_PATH),
        "failure_message": str(primary.get("failure_message") or "assertion failure"),
    }


def pytest_junit_context(evidence_file: Path) -> Dict[str, Any]:
    try:
        junit_text = evidence_file.read_text(encoding="utf-8")
    except OSError as exc:
        raise SystemExit(f"Evidence file not readable: {evidence_file} ({exc})") from exc

    parsed = parse_junit_xml(junit_text)
    if int(parsed.get("test_failures") or 0) <= 0:
        raise SystemExit("ER067_EXPECTED_TEST_FAILURE_NOT_OBSERVED")

    return {
        "test_framework": "pytest",
        "test_result_format": "junit_xml",
        "test_total": int(parsed.get("test_total") or 1),
        "test_passed": max(0, int(parsed.get("test_total") or 1) - int(parsed.get("test_failures") or 0) - int(parsed.get("test_errors") or 0)),
        "test_failures": int(parsed.get("test_failures") or 0),
        "test_errors": int(parsed.get("test_errors") or 0),
        "test_skipped": 0,
        "tests_failed": int(parsed.get("test_failures") or 0),
        "failure_count": int(parsed.get("test_failures") or 0),
        "scan_passed": False,
        ...
        "failed_test_name": str(parsed.get("failed_test_name") or CONTROLLED_TEST_NAME),
        "failed_test_file": str(parsed.get("failed_test_file") or FIXTURE_PATH),
        "failure_message": str(parsed.get("failure_message") or "assertion failure"),
        "reason": "ci_test_failed",
    }


SUPPORTED_EVENT_BY_FORMAT = {
    "pytest-junit": "ci.test.failed",
}
...
def context_from_evidence(evidence_file: Path, evidence_format: str) -> Tuple[str, Dict[str, Any]]:
    ...
    if fmt == "pytest-junit":
        return "ci.test.failed", pytest_junit_context(evidence_file)
...
def main() -> None:
    ...
    print(json.dumps({
        "event_type_id": event_type_id,
        "failed_test_name": context.get("failed_test_name"),
        "failed_test_file": context.get("failed_test_file"),
        "tests_failed": context.get("tests_failed"),
