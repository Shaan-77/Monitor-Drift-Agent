#!/usr/bin/env python3
"""Build ER-069 workflow-skipped evidence from the exact GitHub target job."""
...
EVENT_TYPE_ID = "ci.workflow.skipped"
TARGET_JOB_DISPLAY_NAME = "ZeroUI FM-1 ER-069 Required Release Workflow"
TARGET_JOB_KEY = "zeroui-fm1-er069-required-release"
WORKFLOW_PATH = ".github/workflows/zeroui-fm1-simulator-er069.yml"
WORKFLOW_NAME = "ZeroUI FM-1 ER-069 Workflow Skipped"
EVIDENCE_PATH = Path(".zeroui-simulator/ci/current/er069/artifact/workflow-skipped-evidence.json")
...
def validate_target_job_completed_skipped(job: dict[str, Any]) -> None:
    status = str(job.get("status") or "").strip().lower()
    conclusion = str(job.get("conclusion") or "").strip().lower()
    if conclusion == "cancelled":
        raise SystemExit("ER069_TARGET_WORKFLOW_CANCELLED")
    if conclusion in {"failure", "failed"}:
        raise SystemExit("ER069_TARGET_WORKFLOW_FAILED")
    if conclusion in {"timed_out", "timeout", "stale"}:
        raise SystemExit("ER069_TARGET_WORKFLOW_TIMED_OUT")
    if conclusion in {"success", "passed"}:
        raise SystemExit("ER069_EXPECTED_WORKFLOW_SKIP_NOT_OBSERVED")
    if status != "completed":
...
def build_workflow_skipped_evidence(
    *,
...
def main() -> None:
    ...
    if classified != "skipped":
        raise SystemExit("ER069_TARGET_JOB_VERIFICATION_FAILED")

    jobs = fetch_workflow_jobs(run_id)
    matches = select_target_jobs_by_display_name(jobs, TARGET_JOB_DISPLAY_NAME)
    if len(matches) == 0:
        raise SystemExit("ER069_TARGET_JOB_NOT_IDENTIFIED")
    if len(matches) != 1:
        raise SystemExit("ER069_TARGET_JOB_IDENTITY_AMBIGUOUS")

    target_job = matches[0]
    evidence = build_workflow_skipped_evidence(
    ...
    print("ZeroUI ER-069 workflow skipped evidence:")
