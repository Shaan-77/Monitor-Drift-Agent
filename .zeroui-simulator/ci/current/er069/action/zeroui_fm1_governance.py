...
SCENARIO_PATH = Path(".zeroui-simulator/ci/current/er069/scenario.json")
EXPECTED_ER = "ER-069"
...
def github_workflow_skipped_context(evidence: Any) -> Dict[str, Any]:
    ...
    if not isinstance(data["workflow_skipped"], bool) or data["workflow_skipped"] is not True:
        raise SystemExit("workflow_skipped must be true")
    if not isinstance(data["workflow_failed"], bool) or data["workflow_failed"] is not False:
        raise SystemExit("workflow_failed must be false")
    if str(data["workflow_conclusion"]).strip().lower() != "skipped":
        raise SystemExit("workflow_conclusion must be skipped")
    ...
    if str(data["target_job_name"]) != TARGET_JOB_DISPLAY_NAME:
        raise SystemExit("target_job_name must match the managed ER-069 target job display name")
    if str(data["target_job_conclusion"]).strip().lower() != "skipped":
        raise SystemExit("target_job_conclusion must be skipped")
    ...
    return {
        ...
        "target_job_key": str(data["target_job_key"]),
        "target_job_name": str(data["target_job_name"]),
        "target_job_id": target_job_id,
        "target_job_conclusion": "skipped",
...
SUPPORTED_EVENT_BY_FORMAT = {
    "github-workflow-skipped": "ci.workflow.skipped",
}


def context_from_evidence(evidence_file: Path, evidence_format: str) -> Tuple[str, Dict[str, Any]]:
    fmt = evidence_format.strip().lower()
    if fmt == "github-workflow-skipped":
        return "ci.workflow.skipped", github_workflow_skipped_context(read_json(evidence_file))
