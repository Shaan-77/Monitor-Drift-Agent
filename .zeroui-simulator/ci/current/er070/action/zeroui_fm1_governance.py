...
SCENARIO_PATH = Path(".zeroui-simulator/ci/current/er070/scenario.json")
EXPECTED_ER = "ER-070"
...
def github_artifact_build_failure_context(evidence: Any) -> Dict[str, Any]:
    ...
    if str(data.get("workflow_family") or "") == "release_as_code":
        raise SystemExit("ER070_ROUTING_CONTEXT_INVALID")
    if str(data["build_family"]) != BUILD_FAMILY:
        raise SystemExit("build_family must be artifact_build")
    if str(data["check_family"]) != CHECK_FAMILY:
        raise SystemExit("check_family must be artifact_build")
    if str(data["artifact_ref"]) != ARTIFACT_REF:
        raise SystemExit("artifact_ref must match the managed ER-070 artifact reference")
    if str(data["artifact_type"]) != ARTIFACT_TYPE:
        raise SystemExit("artifact_type must be python_wheel")
    if data.get("artifact_digest"):
        raise SystemExit("artifact_digest must not be present for ER-070 evidence")
    if not isinstance(data["build_passed"], bool) or data["build_passed"] is not False:
        raise SystemExit("build_passed must be false")
    if not isinstance(data["artifact_build_failed"], bool) or data["artifact_build_failed"] is not True:
        raise SystemExit("artifact_build_failed must be true")
    ...
    if str(data["reason"]) != ROUTING_REASON:
        raise SystemExit("reason must be ci_artifact_build_failed")
    ...
    return {
        ...
        "scan_passed": False,
        "failure_deterministic": bool(data.get("failure_deterministic", True)),
        "failure_type": str(data.get("failure_type") or "syntax_error"),
        "failure_stage": str(data.get("failure_stage") or "source_compile"),
        "failed_source_file": str(data.get("failed_source_file") or DRIFT_MONITOR_PATH),
        "failure_message": str(data["failure_message"]),
        "reason": ROUTING_REASON,
        "artifact_build_system": str(data.get("artifact_build_system") or "python_build"),
        "build_tool": str(data.get("build_tool") or "python_m_build"),
        "compile_exit_code": data.get("compile_exit_code"),
        "build_exit_code": data.get("build_exit_code"),
    }


def validate_er070_routing_context_before_post(event_type_id: str, context: Dict[str, Any]) -> None:
    if event_type_id != "ci.artifact.build_failed":
        raise SystemExit("ER070_ROUTING_CONTEXT_INVALID")
    if str(context.get("workflow_family") or "") == "release_as_code":
        print("ER-070 artifact-build-failure evidence cannot use the release_as_code routing family because that family belongs to ER-006.")
        raise SystemExit("ER070_ROUTING_CONTEXT_INVALID")
    if context.get("required_gate") is not True:
        raise SystemExit("ER070_ROUTING_CONTEXT_INVALID")
    if str(context.get("build_family") or "") != BUILD_FAMILY:
        raise SystemExit("ER070_ROUTING_CONTEXT_INVALID")
    if str(context.get("check_family") or "") != CHECK_FAMILY:
        raise SystemExit("ER070_ROUTING_CONTEXT_INVALID")
    if context.get("build_passed") is not False:
        raise SystemExit("ER070_ROUTING_CONTEXT_INVALID")
    if context.get("artifact_build_failed") is not True:
        raise SystemExit("ER070_ROUTING_CONTEXT_INVALID")
    if context.get("artifact_digest"):
        raise SystemExit("ER070_ROUTING_CONTEXT_INVALID")


SUPPORTED_EVENT_BY_FORMAT = {
    "github-artifact-build-failure": "ci.artifact.build_failed",
}


def context_from_evidence(evidence_file: Path, evidence_format: str) -> Tuple[str, Dict[str, Any]]:
    fmt = evidence_format.strip().lower()
    if fmt == "github-artifact-build-failure":
        return "ci.artifact.build_failed", github_artifact_build_failure_context(read_json(evidence_file))
...
def enforce_response(response: Dict[str, Any]) -> None:
    ...
    if str(er or "").strip().upper() != EXPECTED_ER:
        ...
        print(f"::error title=ER070_MAPPED_ER_MISMATCH::mapped ER must be {EXPECTED_ER}, got {er}.")
        raise SystemExit(1)
    ...
    if decision in {"hard_block", "soft_block", "action_required"}:
        if blocker_count < 1:
            write_summary("❌ ZeroUI governance check failed.", "Active blocker missing", summary_fields)
            print("::error title=ZeroUI governance check failed::Active blocker was not created.")
            raise SystemExit(1)
