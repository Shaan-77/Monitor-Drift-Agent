...
def artifact_integrity_context(evidence: Any) -> Dict[str, Any]:
    ...
    # Validate types
    if not isinstance(data["artifact_path"], str):
        raise SystemExit("artifact_path must be a string")
    if not isinstance(data["artifact_present"], bool):
        raise SystemExit("artifact_present must be a boolean")
    if not isinstance(data["artifact_digest"], str):
        raise SystemExit("artifact_digest must be a string")
    if not isinstance(data["source_built_in_ci"], bool):
        raise SystemExit("source_built_in_ci must be a boolean")
    if not isinstance(data["signature_present"], bool):
        raise SystemExit("signature_present must be a boolean")
    if not isinstance(data["signature_valid"], bool):
        raise SystemExit("signature_valid must be a boolean")
    if not isinstance(data["provenance_present"], bool):
        raise SystemExit("provenance_present must be a boolean")
    if not isinstance(data["provenance_valid"], bool):
        raise SystemExit("provenance_valid must be a boolean")
    if not isinstance(data["validation_passed"], bool):
        raise SystemExit("validation_passed must be a boolean")
    if not isinstance(data["scan_passed"], bool):
        raise SystemExit("scan_passed must be a boolean")
    if not isinstance(data["reason"], str):
        raise SystemExit("reason must be a string")
    if not isinstance(data["attestation_provider"], str):
        raise SystemExit("attestation_provider must be a string")
    if not isinstance(data["verification_tool"], str):
        raise SystemExit("verification_tool must be a string")
    if not isinstance(data["verification_status"], str):
        raise SystemExit("verification_status must be a string")

    status = data["verification_status"].strip().lower()
    if status not in {"missing", "invalid", "verified"}:
        raise SystemExit(f"Invalid verification_status: {status}")

    return {
        "artifact_path": data["artifact_path"],
        "artifact_present": data["artifact_present"],
        "artifact_digest": data["artifact_digest"],
        "source_built_in_ci": data["source_built_in_ci"],
        "signature_present": data["signature_present"],
        "signature_valid": data["signature_valid"],
        "provenance_present": data["provenance_present"],
        "provenance_valid": data["provenance_valid"],
        "validation_passed": data["validation_passed"],
        "scan_passed": data["scan_passed"],
        "reason": data["reason"],
        "attestation_provider": data["attestation_provider"],
        "verification_tool": data["verification_tool"],
...
SUPPORTED_EVENT_BY_FORMAT = {
    ...
    "artifact-integrity-evidence": "ci.artifact.unsigned",
}
...
def context_from_evidence(evidence_file: Path, evidence_format: str) -> Tuple[str, Dict[str, Any]]:
    ...
    if fmt == "artifact-integrity-evidence":
        return "ci.artifact.unsigned", artifact_integrity_context(read_json(evidence_file))
